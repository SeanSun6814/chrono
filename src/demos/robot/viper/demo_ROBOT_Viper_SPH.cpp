...
void CreateSolidPhase(ChSystemNSC& mphysicalSystem, ChSystemFsi& myFsiSystem, std::shared_ptr<fsi::SimParams> paramsH) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        std::string BCE_path = GetChronoDataFile("fsi/demo_BCE/moonranger-med-rot-ext-2.25.txt");
        if (i == 0 || i == 2) {
            myFsiSystem.AddBceFile(paramsH, wheel_body, BCE_path, ChVector<>(0, 0.1, -0.1 * 0), Q_from_AngZ(CH_C_PI),
                                   1.0, true);
        } else {
            myFsiSystem.AddBceFile(paramsH, wheel_body, BCE_path, ChVector<>(0, -0.1, -0.1 * 0), QUNIT, 1.0, true);
        }
    }
}
...
void SaveParaViewFiles(ChSystemFsi& myFsiSystem,
    ...
    if (pv_output && std::abs(mTime - (next_frame)*frame_time) < 1e-5) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            auto mmesh = chrono_types::make_shared<ChTriangleMeshConnected>();
            std::string obj_path = GetChronoDataFile("robot/viper/obj/MoonRanger_Wheel_medium_rot.obj");
            double scale_ratio = 1.0;
            mmesh->LoadWavefrontMesh(obj_path, false, true);
            auto transformVector = ChVector<>(0, -0.1, -0.1*0);
